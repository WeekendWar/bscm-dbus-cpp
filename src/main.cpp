...
void printDeviceInfo(const BluetoothDevice& device, int index)
{
  ...
  if (!device.services.empty())
  {
    ...
    for (size_t i = 0; i < device.services.size() && i < 3; i++)
    {
      ...
      if (i < device.services.size() - 1 && i < 2)
        std::cout << ", ";
    }
...
void printCharacteristicInfo(const BluetoothCharacteristic& characteristic,
                             int                            index)
{
...
std::vector<uint8_t> parseHexString(const std::string& hexStr)
{
  ...
  for (size_t i = 0; i < cleanStr.length(); i++)
  {
    ...
    {
      if (i < cleanStr.length() - 1 && cleanStr[i] == '0' &&
          cleanStr[i + 1] == 'x')
      {
        i++;  // Skip the 'x'
        continue;
...
int main()
{
  ...
  manager.setNotificationCallback([](const std::string&          charPath,
  ...
  while (true)
  {
    ...
    switch (choice)
    {
      ...
      case 2:
      {
        std::cout << "\nEnter desired service UUIDs (comma-separated, or "
                     "'none' to clear): ";
        std::cin.ignore();
        std::string input;
      ...
      case 6:
      {
        ...
        if (connectedDevices.empty())
        {
          std::cout << "No connected devices. Please connect to a device first."
                    << std::endl;
          break;
        }
        ...
        auto characteristics =
          manager.getCharacteristics(connectedDevices[deviceChoice].path);
        ...
        while (true)
        {
          ...
          std::cout << "Device: " << connectedDevices[deviceChoice].name
                    << std::endl;
          ...
          switch (action)
          {
            ...
            {
              std::cout
                << "Enter hex data to write (e.g., '01 02 03' or '010203'): ";
              std::cin.ignore();
              std::string hexInput;
