...
public:
    ...
    std::vector<BluetoothCharacteristic> getCharacteristics(const std::string& devicePath);
    ...
    bool writeCharacteristic(const std::string& characteristicPath, const std::vector<uint8_t>& data);
    std::vector<uint8_t> readCharacteristic(const std::string& characteristicPath);
    
    // Notification handling
    void processNotifications();
    void setNotificationCallback(std::function<void(const std::string&, const std::vector<uint8_t>&)> callback);
...
private:
    ...
    void parseDeviceProperties(const std::string& devicePath, BluetoothDevice& device);
    void parseCharacteristicProperties(const std::string& charPath, BluetoothCharacteristic& characteristic);
    bool hasDesiredService(const BluetoothDevice& device);
    static DBusHandlerResult messageFilter(DBusConnection* connection, DBusMessage* message, void* userData);
};
